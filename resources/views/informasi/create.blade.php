...
<head>
    ...
    <title>Tambah Informasi | KnowMS</title>
    <style>
...
<body>
    <div class="container">
        ...
        <p style="color: #666; font-size: 14px; margin-top: -10px;">Lengkapi formulir di bawah ini untuk menambahkan pengetahuan baru.</p>
        ...
        <form method="POST" action="{{ route('informasi.simpan') }}">
            @csrf

            <div class="form-group">
                <label for="judul">Judul Informasi <span style="color:red">*</span></label>
                <input type="text" id="judul" name="judul" value="{{ old('judul') }}" placeholder="Contoh: Panduan Menggunakan Fitur Pencarian">
                @error('judul')
            ...
            <div class="form-group">
                ...
                <textarea id="ringkasan" name="ringkasan" rows="3" placeholder="Tuliskan 1-2 kalimat ringkasan tentang informasi ini...">{{ old('ringkasan') }}</textarea>
                @error('ringkasan')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea id="isi" name="isi" rows="6" placeholder="Tuliskan konten detail informasi secara lengkap...">{{ old('isi') }}</textarea>
                @error('isi')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <input type="text" id="sumber" name="sumber" value="{{ old('sumber') }}" placeholder="Contoh: Dokumentasi Internal / Buku Panduan">
                @error('sumber')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <select id="status" name="status">
                    <option value="published" {{ old('status') == 'published' ? 'selected' : '' }}>Published (Dapat dibaca oleh umum)</option>
                    <option value="draft" {{ old('status') == 'draft' ? 'selected' : '' }}>Draft (Hanya tersimpan di panel admin)</option>
                </select>
                @error('status')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
